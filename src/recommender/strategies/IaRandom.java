...
public class IaRandom implements IaStrategies {
    ...
    public String ia(Grids grids, int pronfondeurMax) {
        ...
        if (sol.size() != 0) {
            ...
            int nb = (int) (Math.random() * listefils.size()); // on choisi aléatoirement un noeud parmis les plus optimale
            grids.affichage();
            if (listefils.get(nb).getlisteaction().get(0).getAction().equals("Deplacement droite")) {
                grids.move(RIGHT);
            } else if (listefils.get(nb).getlisteaction().get(0).getAction().equals("Deplacement gauche")) {
                grids.move(LEFT);
            } else if (listefils.get(nb).getlisteaction().get(0).getAction().equals("Deplacement haut")) {
                grids.move(UP);
            } else if (listefils.get(nb).getlisteaction().get(0).getAction().equals("Deplacement bas")) {
                grids.move(DOWN);
            } else if (listefils.get(nb).getlisteaction().get(0).getAction().equals("Deplacement etages superieurs")) {
                grids.move(FRONT);
            } else {
                grids.move(BACK);
            }
            return listefils.get(nb).getlisteaction().get(0).getAction();
        }
    }
}